      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-700 max-w-3xl mx-auto">
            ...
            competitive exams and build a strong academic foundation.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredCourses.map((course) => (
            <CourseCard
              key={course.id}
              course={course}
              onEnroll={() => scrollToSection("contact")}
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 flex flex-col h-full">
      ...
      <div className="px-6 pb-6">
        <button
          onClick={onEnroll}
          className="cursor-pointer w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-md flex items-center justify-center transition-colors duration-300 font-medium"
        >
          Enroll Now
